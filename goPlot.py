...
def getcolor(cvar, col_data):
    ...
    if (len(choicesCo) < len(basecolors0)):
        colorD = {item: basecolors0[choicesCo.index(
            item)] for item in choicesCo}
        colorlist = [colorD[item] for item in col_data]
        lpatches = [mpatches.Patch(
            color=colorD[item], label=cvar + ', ' + str(item)) for item in colorD.keys()]
    else:
        cmap = plt.cm.plasma
        # colorNos = [choicesCo.index(item) for item in col_data]
        colorD = {item: cmap(choicesCo.index(item)/len(choicesCo))
                  for item in choicesCo}
        colorlist = [colorD[item] for item in col_data]
        lpatches = [mpatches.Patch(
            color=colorD[item], label=cvar + ', ' + str(item)) for item in choicesCo]
...
# ROWLENGTH and getRowCol controls the display of numeric variables
# to be plotted
ROWLENGTH = 4
...
class goPlot(tk.Toplevel):
    def __init__(self):
        ...
        self.clear_button = tk.Button(
            self.f0a, text='Clear Last Variable', command=self.clearIt)
    ...
    def saveData(self, *args):
        file_path = filedialog.asksaveasfilename(
            ...
            initialfile=f"DG_plotdata_{str(datetime.now()).replace(' ', '_')}.csv"
        )
...
class gVar(tk.Frame):
    def __init__(self, parent, varname=None):
        ...
        self.gxlnest = ttk.Combobox(
            self.mframe2, values=estvals, textvariable=self.gxlineest, width=8)
    ...
    def doPlotParms(self, *args):  # set up the UI for extra information for each type of plot
        ...
        if plot_type == 'Histogram':
            ...
            if len(self.master.master.v1list) > 2:
                for idx, item in enumerate(self.master.master.v1list):
                    item.gxlnstyle['values'] = ['-'] + \
                        ['surface', 'wireframe', 'triangulated']
                    item.clickedGXlnstyle.set('-')
                    if idx < 2:
                        item.mframe.grid_forget()
                        item.mframe2.grid_forget()
...
# Set the parameters for scatterplot, lineplot and regplot
            scatterplotdict = {"x": xv, "y": yv, "hue": huelist, "palette": palettev, "style": mv,
                               "color": ycolor, "s": ysize, "ax": ax, "marker": ymarker}
            ...
            if (showscatter):
                ...
                lstr = "sb.scatterplot(" + ", ".join([str(k)+" = "+str(scatterplotdict[k]) for k in list(scatterplotdict.keys())])+")"
                gdata.code_It(lstr)
                sb.scatterplot(df, **scatterplotdict)

            if (showline):
                # print("lineplot params showline only ***>:\n =======\n",lineplotdict)
                lstr = "sb.lineplot(" + ", ".join([str(k)+" = "+str(scatterplotdict[k]) for k in list(scatterplotdict.keys())])+")"
                gdata.code_It(lstr)
                sb.lineplot(df, **lineplotdict)

            if (showsmooth and polyorder == 0):
                smoothed = sm.nonparametric.lowess(exog=np.array(
                    df[xv]), endog=np.array(df[yv]), frac=loessfrac)
                gdata.log_It(loess_msg)
                dfsmooth = pd.DataFrame(
                    {"xs": smoothed[:, 0], "ys": smoothed[:, 1]})
                lineplotdict["x"] = 'xs'
                lineplotdict["y"] = 'ys'
                lineplotdict["estimator"] = None

                # print("lineplot params loess ***>:\n =======\n",", ".join([str(k)+" = "+str(lineplotdict[k]) for k in list(lineplotdict.keys())]))
                lstr = "sb.lineplot(" + ", ".join([str(k)+" = "+str(lineplotdict[k]) for k in list(lineplotdict.keys())]) + ")"
                gdata.code_It(lstr)
                ...
                lstr = "sb.regplot(" + ", ".join([str(k)+" = "+str(regplotdict[k])  for k in list(regplotdict.keys())])+')'
                gdata.code_It(lstr)
...
# hue variable to choose marker types.  So I just ignore it. Colors are enough of a problem to manage.
        ...
        if doreg == 'Yes':
            lstr = "sb.lmplot(" + ", ".join([str(k)+" = "+str(plotdict[k]) for k in list(plotdict.keys())])+")"
            gdata.code_It(lstr)
            g = sb.lmplot(df, **plotdict)
        else:
            lstr = "sb.relplot(" + ", ".join([str(k)+" = "+str(plotdict[k]) for k in list(plotdict.keys())])+")"
            gdata.code_It(lstr)
    ...
    def plot3DScatter(self):
        ...
        for zitem in self.master.master.v1list[2:]:
            ...
            if showscatter == 'Yes':
                ...
                lstr = f"ax.scatter3D(df[{xv}], df[{yv}], df[{zv}], c= colorlist, " + ", ".join([str(k)+" = "+str(scatterplotparms[k])  for k in list(scatterplotparms.keys())[1:]])+')'
                gdata.code_It(lstr)
        ...
        scatter_legend = ax.legend(
            handles=lpatches, loc='upper right', title='Scatter Plot', bbox_to_anchor=(1.05, 1.1))
        ax.add_artist(scatter_legend)
        # plt.legend1()
        plt.legend(handles=surface_legend_entries, loc='upper left',
                   title='Smoothings', bbox_to_anchor=(0.00, 1.05))
        plt.show()
        return
    ...
    def plotECDF(self):
        ...
        lg_msg = f" Empirical CDF Plot:  x= {xv}, color= {
            cv}, marker= {mv}, rows= {rf}, cols= {cf}, Title: {ptitle}"
        gdata.log_It(lg_msg)
        ...
        if (rf == '-') & (cf == '-'):
            if cv == '-':
                ...
                fg = sb.displot(df, x=xv, hue=cv,
                                kind='ecdf', palette='bright')
        elif (cf == '-') & (rf != '-'):
            if (cv != '-'):
                fg = sb.displot(df, x=xv, hue=cv, row=rf,
                                kind='ecdf', palette='bright')
            else:
                fg = sb.displot(df, x=xv, row=rf, kind='ecdf')
        elif (cf != '-') & (rf == '-'):
            if (cv != '-'):
                fg = sb.displot(df, x=xv, hue=cv, row=cf,
                                kind='ecdf', palette='bright')
            else:
                fg = sb.displot(df, x=xv, row=cf, kind='ecdf')
        else:
            if (cv != '-'):
                fg = sb.displot(df, x=xv, hue=cv, row=rf, col=cf,
                                kind='ecdf', palette='bright')
            else:
    ...
    def plotBoxplot(self):
        ...
        if ditherparm == 'Yes':
            np.random.seed(RSEED)
            fg.map_dataframe(sb.stripplot, y=yvp, x=gvp, jitter=True,
                             alpha=0.6, size=DOTSIZE, color='black')
    ...
    def plotHistogram(self, *args):  # finally we're going to make a plot happen!
        ...
        if (rf == '-') & (cf == '-'):
            if cv == '-':
                ...
                fg = sb.displot(df, x=xv,  kde=KDE, bins=nbins,
                                facet_kws=dict(margin_titles=True))
            else:
                # ax = sb.histplot(df, x= xv, hue = cv, kde = KDE, fill = True, palette='bright', ec = 'black', lw = 1, bins = nbins)
                fg = sb.displot(df, x=xv, hue=cv, kde=KDE, bins=nbins,
                                palette='bright', facet_kws=dict(margin_titles=True))
        elif (cf == '-') & (rf != '-'):
            if (cv != '-'):
                fg = sb.displot(df, x=xv, hue=cv, row=rf, kde=KDE, bins=nbins,
                                palette='bright', facet_kws=dict(margin_titles=True))
            else:
                fg = sb.displot(df, x=xv, row=rf, kde=KDE,
                                bins=nbins, facet_kws=dict(margin_titles=True))
        elif (cf != '-') & (rf == '-'):
            if (cv != '-'):
                fg = sb.displot(df, x=xv, hue=cv, row=cf, kde=KDE, bins=nbins,
                                palette='bright', facet_kws=dict(margin_titles=True))
            else:
                fg = sb.displot(df, x=xv, row=cf, kde=KDE,
                                bins=nbins, facet_kws=dict(margin_titles=True))
        else:
            if (cv != '-'):
                fg = sb.displot(df, x=xv, hue=cv, row=rf, col=cf, kde=KDE, bins=nbins,
                                palette='bright',  facet_kws=dict(margin_titles=True))
            else:
                fg = sb.displot(df, x=xv, row=rf, col=cf, kde=KDE,
                                bins=nbins, facet_kws=dict(margin_titles=True))
