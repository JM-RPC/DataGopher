...
def goModel():
    ...
    #update log record function calls
    fam = ''
    overdstr = ''
    if MODEL_TYPE == 'OLS':
        gdata.code_It(f"smf.ols(formula={MODEL_STRING}, data=df).fit()")
    else:
        if MODEL_TYPE == 'LOGIT':
            fam = 'Binomial'
            ...
            overdstr = f"alpha = {imdl.overdispersion}, "
        gdata.code_It(f"res = smf.glm(formula = {MODEL_STRING}, data = df, family=sm.families.{fam}({overdstr}link=sm.families.links.{MODEL_LINK}())).fit()")
    ...
    if STOP:
        #print(LOGSTR)
        messagebox.showerror(' ',f"Model fit failed. Check log file.")
        return (None, None, imdl.LOGSTR)
