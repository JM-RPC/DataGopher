...
from loess_2d import loess_2d, loess_msg
...
plot_types = ['-','Boxplot', 'Histogram', 'ECDF', '2D Line/Scatter Plot','2D Scatter Grid', '3D Scatter Plot']
...
class goPlot(tk.Toplevel):
    ...
    def varupdate(self,event):
        ...
        for idx, item in enumerate(self.v1list):
            ...
            print(f" idx = {idx}, variable = {item.varname}, plot type = {self.plot_type}")
            if idx == 0: 
                ...
                item.mframe.grid(row = 4, column = 0, rowspan=3, columnspan = 2, sticky='w')
                item.mframe2.grid(row = 7, column = 0, rowspan=3, columnspan = 2, sticky='w')
        ...
        #     self.v1list[1].mframe2.grid(row = 7, column = 0, rowspan=3, columnspan = 2, sticky='w')
        # #if there are no variables in v1list then kill the plot choice window
        # if (len(self.v1list)==0):
        #     for item in self.f3.winfo_children():
        #         item.destroy()
    ...
    def parmupdate(self, event):  #makes the entries in the color variable, marker variable, and facet variables unique
        ...
        marker_var = [item for item in self.marklist0 if item not in current_parms ]
        color_var  = [item for item in self.colorlist0 if item not in current_parms + current_vars]
        facet_var  = [item for item in self.facetlist0 if item not in current_parms]
        ...
        remaining_varlist = [item for item in self.varlist0 if item not in current_parms + current_vars]
...
class gVar(tk.Frame):    
    def __init__(self, parent, varname = None):  
        ...
        self.gxlabcolor = tk.Label(self.mframe,text = "Color:").grid(row = 1, column = 2, sticky = 'w')
        ...
        self.gxsmootherscttrlab = tk.Label(self.mframe,text = "Show scatter plot?").grid(row = 3, column = 0, sticky = 'w')
        self.gxsmootherscatter = tk.StringVar(value='Yes')
        self.gxsmthci1 = tk.Radiobutton(self.mframe, text = "Yes", variable=self.gxsmootherscatter, value = 'Yes').grid(row=3, column = 1, sticky = 'w')
        self.gxsmthci2 = tk.Radiobutton(self.mframe, text = "No", variable=self.gxsmootherscatter, value = 'No').grid(row=3, column = 2, sticky = 'w')
        ...
        self.gxlnstyle = ttk.Combobox(self.mframe2, values = ['-'] + baselinestyles, textvariable = self.clickedGXlnstyle, width = 6)
        ...
        self.gxlncolor = tk.OptionMenu(self.mframe2,self.clickedGXlncolor,*(['-'] + basecolors0))
        ...
        estvals = ['None', 'mean', 'median','loess', 'polynomial']
        self.gxlnest = ttk.Combobox(self.mframe2, values = estvals, textvariable = self.gxlineest, width = 8)
        ...
        self.gxsmoothercilab = tk.Label(self.mframe2,text = "Show CI?").grid(row = 4, column = 0, sticky = 'w')
        self.gxsmootherci = tk.StringVar(value='No')
        self.gxsmthci1 = tk.Radiobutton(self.mframe2, text = "Yes", variable=self.gxsmootherci, value = 'Yes').grid(row=4, column = 1, sticky = 'w')
        self.gxsmthci1 = tk.Radiobutton(self.mframe2, text = "No", variable=self.gxsmootherci, value = 'No').grid(row=4, column = 2, sticky = 'w')
        ...
        self.gxpolyorderlab = tk.Label(self.mframe2, text = "Polynomial order:").grid(row=7, column = 0, sticky = 'w')
        self.gxpolytext = tk.Entry(self.mframe2, textvariable = self.gxpolysmootherorder, width = 4)
        self.gxpolytext.grid(row = 7, column=1, sticky= 'w')
        self.gxloessfrac = tk.StringVar(value = '0.3')
        self.gxloessfraclab = tk.Label(self.mframe2, text = 'Loess Fraction:').grid(row= 7, column = 2, sticky = 'w')
        self.gxloessfractext = tk.Entry (self.mframe2, textvariable= self.gxloessfrac, width = 3)
    ...
    def doPlotParms(self,*args): #set up the UI for extra information for each type of plot
        ...
        if plot_type == 'Histogram':
            if len(self.master.master.v1list) >1:
                for item in self.master.master.v1list[1:]: item.pack_forget()
...
###########################################################################
    ...
    def get2DScatterParms(self):
        ...
        self.gobutton = tk.Button(self.optionsframe, text="Plot It!", command = self.plot2DScatter).grid(row=1,column = 6)
    ...
    def get3DScatterParms(self):
        ...
        self.gobutton = tk.Button(self.optionsframe, text="Plot It!", command = self.plot3DScatter).grid(row=0,column = 6,sticky = 'w')
        self.titlelabel = tk.StringVar(self)
        self.label =tk.Label(self, text = 'Plot Title:').grid(row=0, column=7, sticky = 'w')
        self.gxlabel = tk.Entry(self, textvariable = self.titlelabel, width = 20)
        self.gxlabel.grid(row = 0, column = 8, sticky = 'w')
    ...
    def getBoxplotParms(self):
        self.selected_boxplot_grouping_variable = tk.StringVar(self.optionsframe,value='-')
        self.labelgroup = tk.Label(self.optionsframe, text="Box Plot Grouping Variable:").grid(row = 0, column = 0)
        self.coptsgroup = ttk.Combobox(self.optionsframe, values = ['-'] + self.master.master.colorlist0, textvariable = self.selected_boxplot_grouping_variable)
    ...
    def getHistogramParms(self):
        ...
        self.nobins_entry = tk.Entry(self.optionsframe,textvariable = self.nobins,width = 6 )
...
###########################################################################
    ...
    def plot2DScatter(self):
        ...
        vars = [xv, cv, mv]
        if len(self.master.master.v1list) >= 2:
            nuvars = [item.varname for item in self.master.master.v1list[1:]]
            vars = vars + nuvars
        ...
        for idx, item in enumerate(self.master.master.v1list[1:]):
            ...
            ycolor = item.clickedGXcol.get()     # the color for the marker for y overrides cv (above)
            ysize = item.gxlsz.get()             # the size for the marker for y (currently controls all feature sizes)
            vystyl = item.clickedGXlnstyle.get() # the line style for y if ='-' then no line
            lcolor = item.clickedGXlncolor.get() # the line color for y
            ltemp = item.gxlineest.get()         # the estimator for y when multiple point at the same x
            showscatter = item.gxsmootherscatter.get()      #boolean True = show scatter Fales = don't
            polyorder = int(item.gxpolysmootherorder.get()) #order for a polynomial smoothin (0 none, 1 OLS, 2 etc..)
...
##Set the parameters for scatterplot, lineplot and regplot            
            scatterplotdict = { "x":xv, "y":yv, "hue": cv, "palette": palettev, "style": mv,
                           "color": ycolor, "s": ysize, "ax": ax, "marker":ymarker, "color":ycolor}
            
            lineplotdict = {"x":xv, "y":yv, "color":lcolor, "linewidth":ysize/5, "ax": ax , "label": yv,
                            "markers": False, "estimator": ltemp, "errorbar": None, "linestyle": vystyl}

            regplotdict = {"x":xv, "y": yv, "label": yv, "scatter": False, "ax":ax, "label" : yv,
            ...
            if (showscatter): 
                ...
                lstr = "scatterplot params: ***>\n =======\n" + ", ".join([str(k)+" = "+str(scatterplotdict[k]) for k in list(scatterplotdict.keys())])
                gdata.log_It(lstr)
                sb.scatterplot(df,**scatterplotdict)
                
            if (showline): 
                #print("lineplot params showline only ***>:\n =======\n",lineplotdict)
                lstr = "lineplot params showline only ***>:\n =======\n" + ", ".join([str(k)+" = "+str(scatterplotdict[k]) for k in list(scatterplotdict.keys())])
                gdata.log_It(lstr)
                sb.lineplot(df,**lineplotdict)
            ...
            if (showsmooth and polyorder == 0): 
                ...
                #print("lineplot params loess ***>:\n =======\n",", ".join([str(k)+" = "+str(lineplotdict[k]) for k in list(lineplotdict.keys())]))
                lstr = "lineplot params loess ***>:\n =======\n" + ", ".join([str(k)+" = "+str(lineplotdict[k]) for k in list(lineplotdict.keys())])
                gdata.log_It(lstr)
                # Compute a lowess smoothing of the data
                sb.lineplot(data = dfsmooth, **lineplotdict)
            elif (showsmooth and polyorder >=1):
                #print("regplot params: ploynomial ***> \n =======\n",", ".join([str(k)+" = "+str(regplotdict[k]) for k in list(regplotdict.keys())]))
                lstr = "regplot params: ploynomial ***> \n =======\n" + ", ".join([str(k)+" = "+str(regplotdict[k]) for k in list(regplotdict.keys())])
                gdata.log_It(lstr)
                sb.regplot(df,**regplotdict)
                
            bdl = float(item.gxlb.get())
            bdu = float(item.gxub.get())
            if ylower > bdl : ylower = bdl
            if yupper < bdu:  yupper = bdu
                        
        ax.set_xlabel(xlab)
        ax.set_ylabel(ylab)
        ax.legend(bbox_to_anchor=(.95, 1), loc='upper left', borderaxespad=0)
        #ax.legend()
...
#### hue variable to choose marker types.  So I just ignore it. Colors are enough of a problem to manage.
        ...
        #now do the plot
            if doreg == 'Yes':
                if (rf != '-') & (cf != '-'):
                    ...
                    g = sb.lmplot(df, x = xv, y= yv, hue = cv, palette = palettenu, scatter_kws={"s": ysize})                                       
            else: 
                if (rf != '-') & (cf != '-'):
             ...
             if doreg == 'Yes':
                 if (rf != '-') & (cf != '-'):
                     ...
                     g = sb.lmplot(df, x = xv, y= yv, scatter_kws={"s": ysize})                                       
    ...
    def plot3DScatter(self):
        ...
        #global plot choices
        cv = str(self.master.master.selected_color_variable.get()) #dot color variable
        ...
        if cv != '-':
            colorlist = list(df[cv])
            colorD, colorlist ,cvpatches= getcolor(cv, list(df[cv]))                         
            #ax.legend(handles=lpatches, loc ='upper left', bbox_to_anchor=(1.05, 1))
        ...
        #now plot the additional Z series
        surface_legend_entries= []
        for zitem in self.master.master.v1list[2:]:
            zv = zitem.varname
            ztemp = zitem.clickedGXmkr.get()
            
            zcolor = zitem.clickedGXcol.get()     # the color for the marker for y overrides cv (above)
            zsize = zitem.gxlsz.get()             # the size for the marker for y (currently controls all feature sizes)
            zstyl = zitem.clickedGXlnstyle.get() # the surface style for y if ='-' then no surface
            lcolor = zitem.clickedGXlncolor.get() # the line color for y
            ltemp = zitem.gxlineest.get()         # the estimator for y when multiple point at the same x
            showscatter = zitem.gxsmootherscatter.get()      #boolean True = show scatter False = don't
            polyorder = int(zitem.gxpolysmootherorder.get()) #order for a polynomial smoothin (0 none, 1 OLS, 2 etc..)
            ...
            if showscatter  == 'Yes':
                ...
                colorchoice = None                
                #set up the do coloring vectors
                ...
                if colorlist is not None: color_variable = True
                ...
                #Do the scatter plot
                scatterplotparms = {'c':colorlist, 'color':colorchoice, 'marker':zmarker, 's':zsize, 'alpha': 0.8}
                #print(scatterplotparms)
                ax.scatter3D(df[xv], df[yv], df[zv], **scatterplotparms)

                #ax.scatter3D(df[xv], df[yv], df[zv], c = colorlist, color = colorchoice, marker=zmarker, label = zlegend, s=zsize, alpha=0.8)
            ...
            if zstyl != '-':
                ...
                if ltemp == 'polynomial':
                    ...
                    # Linear regression
                    #note: intercept added to design matrix (input features)
                    ...
                    outdict = dict(zip(poly.get_feature_names_out(), model.coef_.round(4)))
                    pstlist = [str(item) +  "=" + str(outdict[item]) for item in outdict.keys()]
                    gdata.log_It("\n".join(pstlist))
                    #log fit
                    gdata.log_It(f"R-squared: {model.score(poly.transform(input_pts), zin):.3f}")
                ...
                elif ltemp == 'loess': #we'll get to this one later
            ...
            if zstyl == 'triangulated':
                 ...
                 try:
                     ...
                     tk.messagebox.showerror(" ", f"Triangulated surface plot failed! Error:{er}")
            ...
            if zsurf is not None: surface_legend_entries.extend([zsurf])    
            #upper and lower bounds
            zlb = float(zitem.gxlb.get())
            zub = float(zitem.gxub.get())            
            if zlower > zlb: zlower = zlb
            if zupper < zub: zupper = zub
        ...
        if color_variable: lpatches = cvpatches + lpatches
        ...
        #plt.legend1()
        plt.legend(handles=surface_legend_entries, loc='upper right', title='Smoothings', bbox_to_anchor=(-.05, 1.1))
        plt.show()
        return
    ...
    def plotBoxplot(self):
        ...
        #now do the plotting
        try:
            fg = sb.catplot(df, x=gvp, y=yvp, hue = cvp, row = rfp, col = cfp, palette=palettep, kind = 'box', showfliers=fliersp,)
        except Exception as er:
...
if __name__ == "__main__":
    ...
    class solo(tk.Tk):
        def __init__(self):
            ...
            self.goPlt  = tk.Button(self, text = "Plot Data", command = self.pltD).pack(side = tk.LEFT, padx = 10)
