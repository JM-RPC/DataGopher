...
class RegressionApp(tk.Toplevel):
    def __init__(self):
        ...
        self.clickedLink = tk.StringVar(self,'')
    ...
    def build_frames(self):
        ...
        #over_dispLabel.grid(row=RRow, column=6, sticky='e')
        ...
        #self.linklab.grid(row=RRow, column=9, sticky='e')
        self.linklab.grid_forget()
        self.link = ttk.Combobox(self.mf2, values=links, textvariable=self.clickedLink)
        #self.cx.grid(row=RRow, column=10, sticky='w')
    ...
    def model_choice(self, *args):
        ...
        if mdl == 'NEGATIVE BINOMIAL':
            ...
            self.linklab.grid(row = RRow, column = 9,sticky='w')
            self.link.grid(row = RRow, column = 10, sticky = "w")
            ...
            self.over_disp_ent.grid_forget()
            self.over_dispParmLabel.grid_forget()
            self.clickedLink.set('InversePower')
        elif (mdl == 'POISSON'):
            self.linklab.grid(row = RRow, column = 9,sticky='w')
            self.link.grid(row = RRow, column = 10, sticky  = "w")
...
####################################################################
            ...
            self.update_option_menu_vars(mdlvars)
    ...
    def dopredict(self, *args):
        ...
        if (cvpredict != '') and (cvpredict != '-'): 
            sb.scatterplot(imdl.fitdata, x = xvpredict, y = res_frame['mean'], hue = cvpredict, palette = 'bright',s = dsize )
        else:
            sb.scatterplot(imdl.fitdata, x = xvpredict, y = res_frame['mean'], color = 'blue', s = dsize)
        if imdl.model_type in ['LOGIT', 'PROBIT']:
    ...
    def modelplot(self):
        ...
        if len(imdl.indvars)==1:
            ...
            if (cv == '-') or (cv == ''):
                ...
                colorlist ,lpatches= getcolor(list(colordat))
                colorvar,colorD = grp.getcolorM(colordat)
                ax.scatter(xdat, ydat, zdat ,marker='o', c = colorvar, s = dsize)
                if colorD != {}:
                    handles , labels = ax.get_legend_handles_labels()
                    for item in colorD.keys():
                        patch = mpatches.Patch(color = colorD[item], label = item)
